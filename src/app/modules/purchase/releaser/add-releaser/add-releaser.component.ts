...
@Component({
  selector: 'app-add-releaser',
  templateUrl: './add-releaser.component.html',
  styleUrls: ['./add-releaser.component.css']
})
export class AddReleaserComponent implements OnInit, OnDestroy {
  ...
  constructor(
    private service: LiberadorService,
    ...
    this.service.update(this.form.value, Number(this.id))
      ...
      .subscribe(
        (res) => {
          ...
          this.router.navigate(['/Compras/Liberadores'], { relativeTo: this.route })
        },
        (err) => {
          this.alert.error("Ops... Aconteceu um probleminha!");
          this.loading = false
